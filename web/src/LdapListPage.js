...
class LdapListPage extends React.Component {
    ...
    LdapBackend.addLdap(newLdap)
      .then((res) => {
          if (res.status === "ok") {
            ...
            this.setState((prevState) => {
              prevState.ldaps = Setting.prependRow(prevState.ldaps, res.data2);
              return prevState;
            })
          } else {
            Setting.showMessage("error", res.msg);
  ...
  deleteLdap(index) {
    LdapBackend.deleteLdap(this.state.ldaps[index])
      .then((res) => {
          if (res.status === "ok") {
            Setting.showMessage("success", `Delete LDAP server success`);
            this.setState((prevState) => {
              prevState.ldaps = Setting.deleteRow(prevState.ldaps, index);
              return prevState;
            })
          } else {
            Setting.showMessage("error", res.msg);
